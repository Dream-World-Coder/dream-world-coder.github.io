...
- If adjacent elements are compared and swapped then its going to be stable.
	- In [Bubble Sort](https://sortalgo.space/chapters/bubble-sort), we compare adjacent pairs, so for similar values, first item is going to process and place first, thus stable.
	- In [Insertion sort](https://sortalgo.space/chapters/insertion-sort) also adjacent pairs are compared & swapped.
	- In [Merge Sort](https://sortalgo.space/chapters/merge-sort), the merge function compares two sub arrays element by element and puts in a temporary array & later copies them. During the merge step, <u>when two elements are equal, we always take from the left subarray first,</u> and for individual subarrays order is always left to right. This ensures the earlier element is placed first, thus preserving the original order.

- If elements are abruptly picked and compared, then it is going to be unstable.
	- In [Selection Sort](https://sortalgo.space/chapters/selection-sort) we find the minimum element and swap it with the current element. This long-distance swap can move an equal element past its duplicate, breaking the original order.
	...
	- In [Quick Sort](https://sortalgo.space/chapters/quick-sort) <u>the partition index creates abrupt placement and hence instability.</u> Consider the case for similar valued list:
		- sorting: `[(3,a), (2,x), (3,b)]`
	...
	- In [Heap Sort](https://sortalgo.space/chapters/heap-sort) we first build a max-heap from the array and then repeatedly extract the maximum element. During heapify operations, <u>elements are moved based on heap property (parent >= children), not their original order.</u> When building the heap or performing heapify, equal elements can be placed in arbitrary positions.
